  ...
  case 0xff:  // meta_event = 0xFF + <meta_type:1> + <length:v> + <event_data_bytes>
  {
    ...
    switch (eType)
    {
      ...
      case 0x58:  // time signature
      {
        ...
        uint32_t actualPos;
        actualPos=mf->_fd.position();
        mf->_fd.seek(actualPos + mLen - 2);
        mev.data[0] = n;
        mev.data[1] = d;
        mev.data[2] = 0;
        mev.data[3] = 0;
      ...
      default:
      {
        ...
        if (mLen > sizeof(mev.data))
          mf->_fd.seek( (mf->_fd.position() )+ mLen-sizeof(mev.data));
  //    DUMPS("IGNORED");
      }
      break;
    }
...
int MD_MFTrack::load(uint8_t trackId, MD_MIDIFile *mf)
{
  ...
  // track_chunk = "MTrk" + <length:4> + <track_event> [+ <track_event> ...]
  {
    ...
    nfgets(h, MTRK_HDR_SIZE+1, &mf->_fd );
    h[MTRK_HDR_SIZE] = '\0';
  ...
  if (mf->_fd.seek(_startOffset+_length) == -1)
    return(1);
