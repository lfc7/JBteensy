 ...
 * teensy 3.6 version
 * 
 * cause SDfat lib descriptor is already supplied by calling routine
 * 
 * CLAVEAU Olivier / 2019-2020
 * 
 * 
 * *************************************************
...
/*
void MD_MIDIFile::begin(SdFile *psd)
{
  _sd = psd;
}
*/
...
// Load the MIDI file into memory ready for processing
{
  ...
  // header chunk = "MThd" + <header_length:4> + <format:2> + <num_tracks:2> + <time_division:2>
  {
   ...
   // _fd.fgets(h, MTHD_HDR_SIZE+1);
	...
	//_fd.seek(MTHD_HDR_SIZE);
  // read header size
  dat32 = readMultiByte( &_fd, MB_LONG);
  Serial.println(dat32);
