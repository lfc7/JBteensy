...
MCP3017AccelStepper::MCP3017AccelStepper(uint8_t interface, uint8_t pin1, uint8_t pin2, uint8_t pin3, uint8_t pin4) : _accelStepper(interface, pin1, pin2, pin3, pin4) {
  ...
  if( pin2 == 0xff) // mod force pin on mcp and no enable pin
  {
      _pin[1] = pin1 + 8;
      _pin[2] = pin2;
  }
...
void MCP3017AccelStepper::setSpeedMode( bool speedMode )
{
   _speedMode=speedMode;
}

bool MCP3017AccelStepper::getSpeedMode( )
{
   return _speedMode;
}

void MCP3017AccelStepper::update_run()
{
   if( _speedMode )
   {
      runSpeed();
